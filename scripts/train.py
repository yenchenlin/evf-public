...
def main():
    ...
    parser.add_argument("--inner_iters", type=int, default=20, help="number of inner-loop iterations")
    ...
    # Backward compatibility, used to build graph
    train_handle = train_handles[0]
    current_handle = train_handle
    ...
    # Use first train set to build graph
    ...
    with tf.Session(config=config) as sess:
        ...
        print("number of train sets =", len(train_sets))
        ...
        train_handle_evals = [sess.run(train_handle) for train_handle in train_handles]
        val_handle_evals = [sess.run(val_handle) for val_handle in val_handles]
        ...
        for step in range(-1, max_steps - start_step):
            ...
            if should(step, args.inner_iters):
