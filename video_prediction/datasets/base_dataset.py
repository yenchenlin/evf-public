...
class BaseVideoDataset(object):
    ...
    def slice_sequences(self, state_like_seqs, action_like_seqs, example_sequence_length):
        ...
        if (time_shift and self.mode == 'train') or self.hparams.force_time_shift:
            ...
            t_start = 0
        if self.hparams.t_start != -1:
            t_start = self.hparams.t_start
            assert t_start + sequence_length == 60
            assert t_start + self.hparams.context_frames == 50
...
class VarLenFeatureVideoDataset(BaseVideoDataset):
    ...
    def filter(self, serialized_example):
        ...
        condition1 = tf.greater_equal(example_sequence_length, self.hparams.sequence_length)
        if self.hparams.t_start == -1:
            return condition1
        else:
            condition2 = tf.greater_equal(example_sequence_length, self.hparams.t_start + self.hparams.sequence_length)
            return tf.logical_and(condition1, condition2)
